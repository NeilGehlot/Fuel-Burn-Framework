...
def prepare_stint_data(stint_data):
    ...
    Calculate Baseline lap time , clean the data and 
...
def analyse_stint(stint_data,stint_number):
    """
    Analyse a single stint and print 
    stint statistics 
    """
    stint_data  =  stint_data.copy()
    ...
    plt.title(f"Stint {stint_number}: Lap T")
    plt.show()
    return {
        'length':length,
...
def plot_stint(stint_data,stint_number):
    """
    plots the lap time progression  across a stint
    """
...
def  main():
    ...
    for stint in analysis_data["stint_number"].unique():
        ...
        if stint_plot["stint_laps"].max() < 5:
            continue
